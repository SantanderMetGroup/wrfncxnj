...
def get_options():
    ...
    parser.add_option(
        ...
        help="Regular expression to be parsed by python to get the input files"
             "to process",
        metavar="REGEXP"
    )
    parser.add_option(
        "--from-file", dest="filelist", default="",
        help="Text file containing the input files. One per row",
        metavar="FILELIST.txt"
    )
    parser.add_option(
        "--previous-file", dest="prevfile", default="",
        help="Extra input file to be prepended to the input files ONLY for "
             "BACKWARD deaccumulations",
        metavar="WRFNCFILE.nc"
    )
    parser.add_option(
    ...
    parser.add_option(
        ...
        help="Enable discarding files. Currently only the uncommon_size "
             "criteria is implemented", metavar="uncommon_size"
    )
    parser.add_option(
        "-t", "--variable-table", dest="vtable",
        help="Table for translating WRF names into IPCC standard names",
        metavar="variable.table"
    )
    parser.add_option(
        "-a", "--attributes", dest="attributes",
        help="Table for setting the global attributes of the file",
        metavar="atributes.file"
    )
    parser.add_option(
    ...
    parser.add_option(
        ...
        help="Save only one record. Useful to extract fixed fields "
             "(LANDMASK, LANDUSE, ...)"
    )
    parser.add_option(
    ...
    parser.add_option(
        "--time-units", dest="time_units",
        default="hours since 1950-01-01 00:00:00",
        help="Units for the time axis",
        metavar="Days/Hours since YYYY-MM-DD hh:mm:ss "
                "(T as separator also works)"
    )
    parser.add_option(
    ...
    parser.add_option(
        ...
        help="geo_em file to be used. For instance if you already removed "
             "geographic variables from the input files"
    )
    parser.add_option(
        "--fullfile", metavar="wrfout_d0X_allvars.nc", dest="fullfile",
        help="wrfout file to be used for variables not found in the data files."
             " For instance if you removed variables as the eta or soil levels."
    )
    parser.add_option(
    ...
    parser.add_option(
        ...
        help="Write a separated file for each variable and level. (Use always "
             "with --split-variables)"
    )
    parser.add_option(
        "--temp-dir", dest="TEMPDIR",
        help="Temporary directory to write the files while running. They are "
             "copied to the folfer specified by -o or --output-pattern when the"
             " program finishes."
    )
    parser.add_option(
        "--plevs-filter", dest="selected_plevs",
        help="Comma separated list of the pressure levels that we want to save,"
             " in hPa."
    )
    parser.add_option(
        "--slevs-filter", dest="selected_slevs",
        help="Comma separated list of the soil levels that we want to save, in"
