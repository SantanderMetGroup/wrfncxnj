...
import numpy as np
import logging
from wrfncxnj_base import get_oncvar, Constants, compute_mslp, deaccumulate_var
import sys
...
def compute_CLTFR(varobj, onc, wnfiles, wntimes, options):
    ...
    if wnfiles.current.variables.has_key("CLT"):
        ...
        oncvar = get_oncvar(varobj, cldfra, onc, options,
                            out_is_2D_but_in_3D=True)
    return oncvar, copyval


def compute_RH2(varobj, onc, wnfiles, wntimes, options):
...
def compute_TDPS_its90(varobj, onc, wnfiles, wntimes, options):
...
def compute_UER(varobj, onc, wnfiles, wntimes, options):
    ...
    oncvar = get_oncvar(varobj, u, onc, options)
    return oncvar, copyval
...
def compute_RHO2(varobj, onc, wnfiles, wntimes, options):
    """Computes the 2 meter dry air density from 2m temperature, and surface pressure.
    """
    incvar = wnfiles.current.variables["T2"] # K
    tas = incvar[:]
...
def compute_U10(varobj, onc, wnfiles, wntimes, options):
    incvar = wnfiles.current.variables["U10"]
...
def compute_V10(varobj, onc, wnfiles, wntimes, options):
